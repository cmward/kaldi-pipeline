...
SWBD_DIR=$1 ###path to training data directory
KALDI_ROOT=$KALDI
...
if [ ! -d $SWBD_DIR/transcript/swbd_train ]; then
  ...
  [ -f $dir/swbd_train ] \
    || ln -sf $SWBD_DIR/transcript/swbd_train $dir/
fi

# Option A: SWBD dictionary file check
[ ! -f $SWBD_DIR/../lexicon/sw-ms98-dict_patched.text ] && \
  echo  "SWBD dictionary file does not exist" &&  exit 1;

# find sph audio files
find $SWBD_DIR/swbd -iname '*.sph' | sort > $dir/sph.flist
...
}' $SWBD_DIR/transcript/swbd_train/*-trans.text  > $dir/transcripts1.txt
