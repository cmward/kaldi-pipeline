...
LM = pjoin(DATA, 'lm')
FORMAT_LM = pjoin(DATA, 'lang_combo_tg')
EXP = pjoin(PROJECT_ROOT, 'exp')
MAKE_MFCC = pjoin(EXP, 'make_mfcc')
MFCC_TRAIN = pjoin(MAKE_MFCC, 'train')
MONO = pjoin(EXP, 'mono')
MFCC_TEST = pjoin(MAKE_MFCC, 'test')
GRAPH = pjoin(MONO, 'graph_comb_tg')
DECODE_TE = pjoin(MONO, 'decode_test_comb_tg')
UTILS = pjoin(PROJECT_ROOT, 'utils')
